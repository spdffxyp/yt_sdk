...
/**
 * @internal      yt_l2_fdb_ucast_addr_add_dynamic
 * @endinternal
 *
 * @brief         add l2 dynamic fdb table
 * @note          APPLICABLE DEVICES  -Tiger
 * @param[in]     unit                -unit id
 * @param[in]     vid                 -vlan id
 * @param[in]     mac_addr            -yt_types.h
 ...
 * @retval        CMM_ERR_FAIL        -on fail
 */
yt_ret_t  yt_l2_fdb_ucast_addr_add_dynamic(yt_unit_t unit, yt_vlan_t vid, yt_mac_addr_t mac_addr, yt_port_t port, yt_bool_t isLag)
{
    CMM_PARAM_CHK((YT_UNIT_NUM <= unit), CMM_ERR_INPUT);
    CMM_PARAM_CHK(NULL == YT_DISPATCH(unit), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((!(YT_DISPATCH(unit)->is_inited)), CMM_ERR_NOT_INIT);
    ...
    return YT_DISPATCH(unit)->l2_fdb_ucast_addr_add_dynamic(unit, vid, mac_addr, port, isLag);
}
...
/**
 * @internal      yt_l2_mcast_addr_withindex_get
 * @endinternal
 *
 * @brief         get muticast addr entry info by index
 * @note          APPLICABLE DEVICES  -Tiger
 * @param[in]     unit                -unit id
 * @param[in]     index               -index
 * @param[out]    pMcastMac           -mcast mac info,refer to l2_mcastMacAddr_info_t
 * @retval        CMM_ERR_OK          -on success
 * @retval        CMM_ERR_FAIL        -on fail
 * @retval        CMM_ERR_INPUT       -on err input
 * @retval        CMM_ERR_NOT_INIT    -on not init
 * @retval        CMM_ERR_ENTRY_NOT_FOUND -no valid mac entry on this index next
 */
yt_ret_t yt_l2_mcast_addr_withindex_get(yt_unit_t unit, uint16_t index, l2_mcastMacAddr_info_t *pMcastMac)
{
    CMM_PARAM_CHK((YT_UNIT_NUM <= unit), CMM_ERR_INPUT);
    CMM_PARAM_CHK(NULL == YT_DISPATCH(unit), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((!(YT_DISPATCH(unit)->is_inited)), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((NULL == pMcastMac), CMM_ERR_NULL_POINT);

    return YT_DISPATCH(unit)->l2_mcast_addr_withindex_get(unit, index, pMcastMac);
}

/**
 * @internal      yt_l2_mcast_addr_withMacAndVid_get
 * @endinternal
 *
 * @brief         get muticast addr entry info by addr and vlan
 * @note          APPLICABLE DEVICES  -Tiger
 * @param[in]     unit                -unit id
 * @param[in]     vid                 -vlan id
 * @param[in]     mac_addr            -mac address
 ...
 * @retval        CMM_ERR_ENTRY_NOT_FOUND -no valid mac entry on this index next
 */
yt_ret_t yt_l2_mcast_addr_withMacAndVid_get(yt_unit_t unit, yt_vlan_t vid, yt_mac_addr_t mac_addr, l2_mcastMacAddr_info_t *pMcastMac)
{
    CMM_PARAM_CHK((YT_UNIT_NUM <= unit), CMM_ERR_INPUT);
    CMM_PARAM_CHK(NULL == YT_DISPATCH(unit), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((!(YT_DISPATCH(unit)->is_inited)), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((!(IS_MCAST_ADDR(mac_addr.addr))), CMM_ERR_INPUT);
    CMM_PARAM_CHK((NULL == pMcastMac), CMM_ERR_NULL_POINT);

    return YT_DISPATCH(unit)->l2_mcast_addr_withMacAndVid_get(unit, vid, mac_addr, pMcastMac);
}

/**
 * @internal      yt_l2_mcast_addr_withindex_getnext
 * @endinternal
 *
 * @brief         get next valid muticast addr entry info by current index
 * @note          APPLICABLE DEVICES  -Tiger
 * @param[in]     unit                -unit id
 * @param[in]     index               -current index
 * @param[out]    pNext_index         -the index of next vaild entry
 ...
 * @retval        CMM_ERR_ENTRY_NOT_FOUND -no valid mac on this index
 */
yt_ret_t  yt_l2_mcast_addr_withindex_getnext(yt_unit_t unit, uint16_t index, uint16_t *pNext_index, l2_mcastMacAddr_info_t *pMcastMac)
{
    CMM_PARAM_CHK((YT_UNIT_NUM <= unit), CMM_ERR_INPUT);
    CMM_PARAM_CHK(NULL == YT_DISPATCH(unit), CMM_ERR_NOT_INIT);
    CMM_PARAM_CHK((!(YT_DISPATCH(unit)->is_inited)), CMM_ERR_NOT_INIT);
    ...
    return YT_DISPATCH(unit)->l2_mcast_addr_withindex_getnext(unit, index, pNext_index, pMcastMac);
}
