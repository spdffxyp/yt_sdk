...
yt_ret_t fal_tiger_rate_init(yt_unit_t unit)
{
    ...
    gPortRateInit = 0;
    gPortShapeInit = 0;
    osal_memset(gFlowMeterInit, 0, sizeof(gFlowMeterInit));
    osal_memset(gQshapeInit, 0, sizeof(gQshapeInit));
...
static yt_ret_t fal_tiger_rate_igrBandwidthCtrl_cfg_set(yt_unit_t unit, yt_macid_t macId, yt_meterid_t meterId)
{
    ...
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_RATE_MODEf, &cfgTbl, RATE_MODE_BYTE);
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_METER_ENf, &cfgTbl, YT_ENABLE);
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_METER_MODEf, &cfgTbl, METER_MODE_RFC4115);
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_COLOR_MODEf, &cfgTbl, COLOR_BLIND);
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_DROP_COLORf, &cfgTbl, DROP_COLOR_YR);
    HAL_FIELD_SET(METER_CONFIG_TBLm, METER_CONFIG_TBL_CFf, &cfgTbl, CF_MODE_NONE);
...
static yt_ret_t fal_tiger_rate_shaping_port_cfg_set(yt_unit_t unit, yt_macid_t macId)
{
    ...
    HAL_FIELD_SET(PSCH_SHP_CFG_TBLm, PSCH_SHP_CFG_TBL_SHAPER_MODEf, &cfgTbl, RATE_MODE_BYTE);
    /* 1*unit for 2^15 tokens */
...
static yt_ret_t fal_tiger_rate_shaping_queue_cfg_set(yt_unit_t unit, uint8_t qid)
{
    ...
    HAL_FIELD_SET(QSCH_SHP_CFG_TBLm, QSCH_SHP_CFG_TBL_SHAPER_MODEf, &cfgTbl, RATE_MODE_BYTE);
    /* 1*unit for 2^15 tokens */
    cbs = (uint32_t)((FAL_TIGER_DEFAULT_BURST_SIZE << (14 - 2 * FAL_TIGER_DEFAULT_TOKEN_UNIT) >> 16));
