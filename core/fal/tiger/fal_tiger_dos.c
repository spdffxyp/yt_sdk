...
static uint32_t  fal_tiger_dos_tcp_flagsn_set(yt_unit_t unit, yt_dos_type_t type, yt_enable_t enable)
{
    ...
    HAL_FIELD_SET(DOS_TCP_FLAGS0m, DOS_TCP_FLAGS0_DROP_EN, &dos_tcp_flagsn, enable);
...
static uint32_t  fal_tiger_dos_tcp_flagsn_get(yt_unit_t unit, yt_dos_type_t type, yt_enable_t *pEnable)
{
    ...
    HAL_FIELD_GET(DOS_TCP_FLAGS0m, DOS_TCP_FLAGS0_DROP_EN, &dos_tcp_flagsn, &enable);
...
yt_ret_t fal_tiger_dos_drop_en_set(yt_unit_t unit, yt_dos_type_t type, yt_enable_t enable)
{
    ...
    switch (type)
    {
        ...
        case DOS_TYPE_CFI_MISMATCH_DROP:
            ...
            break;
        case DOS_TYPE_UDP_SPORT_EQ_DPORT:
            if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218)
            {
                HAL_FIELD_SET(DOS_CTRLm, DOS_CTRL_UDP_SPORT_EQ_DPORT_DROPf, &dos_ctrl, enable);
                regid = DOS_CTRLm;
            }
            ...
            break;
        case DOS_TYPE_TCP_SPORT_EQ_DPORT:
            if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218)
            {
                HAL_FIELD_SET(DOS_CTRLm, DOS_CTRL_TCP_SPORT_EQ_DPORT_DROPf, &dos_ctrl, enable);
                regid = DOS_CTRLm;
            }
            else if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9215)
            {
...
yt_ret_t fal_tiger_dos_drop_en_get(yt_unit_t unit, yt_dos_type_t type, yt_enable_t *pEnable)
{
    ...
    switch (type)
    {
        ...
        case DOS_TYPE_UDP_SPORT_EQ_DPORT:
            if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218)
            {
                HAL_FIELD_GET(DOS_CTRLm, DOS_CTRL_UDP_SPORT_EQ_DPORT_DROPf, &dos_ctrl, &enable);
            }
            else if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9215)
            {
                ret = CMM_ERR_NOT_SUPPORT;
        ...
        case DOS_TYPE_TCP_SPORT_EQ_DPORT:
            if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218)
            {
                HAL_FIELD_GET(DOS_CTRLm, DOS_CTRL_TCP_SPORT_EQ_DPORT_DROPf, &dos_ctrl, &enable);
            }
            else if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9215)
            {
                ret = CMM_ERR_NOT_SUPPORT;
