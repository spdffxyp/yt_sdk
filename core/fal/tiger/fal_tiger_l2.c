...
yt_ret_t  fal_tiger_l2_fdb_ucast_addr_add_dynamic(yt_unit_t unit, yt_vlan_t vid, yt_mac_addr_t mac_addr, yt_port_t port, yt_bool_t isLag)
{
    yt_port_mask_t portmask;
    cmm_err_t ret = CMM_ERR_OK;
    yt_l2_fdb_op_result_t op_result;
    ...
    if (fal_tiger_l2_fdb_get(unit, vid,  mac_addr,  &fdb_info, &lookup_index) == CMM_ERR_OK)
    {
        CMM_ERR_CHK(fal_tiger_l2_tbl_read(unit, lookup_index, &l2_fdb),ret);
    }

    HAL_FIELD_GET(L2_FDB_TBL_BIN0m, L2_FDB_TBL_BIN0_STATUSf, &l2_fdb, &status_tmp);

    if ((status_tmp == FDB_STATUS_STATIC) || (status_tmp == FDB_STATUS_PENDING))
    {
    ...
    HAL_FIELD_SET(L2_FDB_TBL_BIN0m, L2_FDB_TBL_BIN0_FIDf, &l2_fdb, vid);
    HAL_FIELD_SET(L2_FDB_TBL_BIN0m, L2_FDB_TBL_BIN0_STATUSf, &l2_fdb, FDB_STATUS_MAX_TIME);
    HAL_FIELD_SET(L2_FDB_TBL_BIN0m, L2_FDB_TBL_BIN0_DST_PORT_MASKf, &l2_fdb, macmask.portbits[0]);

    CMM_ERR_CHK(fal_tiger_l2_fdb_op_add_cfg(unit, op_mode, l2_fdb, &op_result), ret);
    if(op_result.lookup_fail)
    {
        return CMM_ERR_ENTRY_NOT_FOUND;
    }
...
yt_ret_t fal_tiger_l2_mcast_addr_withindex_get(yt_unit_t unit, uint16_t index, l2_mcastMacAddr_info_t *pMcastMac)
{
    cmm_err_t ret = CMM_ERR_OK;
    hal_reg_tbl_mode_t table_reg_mode;
    yt_l2_fdb_op_result_t op_result;
    ...
    osal_memset(&table_reg_mode, 0, sizeof(hal_reg_tbl_mode_t));
    osal_memset(&op_result, 0, sizeof(yt_l2_fdb_op_result_t));
    osal_memset(&fdb_info, 0, sizeof(yt_l2_fdb_info_t));
    osal_memset(&mac_addr, 0, sizeof(yt_mac_addr_t));
    ...
    if(HAL_REG_TBL_MODE_NORMAL == table_reg_mode)
    {
        ...
        pMcastMac->vid = fid;
        osal_memcpy(pMcastMac->macaddr.addr, mac_addr.addr, MAC_ADDR_LEN);
        switch(fdb_info.STATUS)
        {
            case FDB_STATUS_PENDING:
                pMcastMac->type = FDB_TYPE_PENDING;
                break;
            case FDB_STATUS_STATIC:
                pMcastMac->type = FDB_TYPE_STATIC;
                break;
            default:
                pMcastMac->type = FDB_TYPE_DYNAMIC;
                break;
        }
    }
...
yt_ret_t fal_tiger_l2_mcast_addr_withMacAndVid_get(yt_unit_t unit, yt_vlan_t vid, yt_mac_addr_t mac_addr, l2_mcastMacAddr_info_t *pMcastMac)
{
    hal_reg_tbl_mode_t table_reg_mode;
    uint16_t lookup_index   = 0;
    cmm_err_t ret           = CMM_ERR_OK;
...
yt_ret_t  fal_tiger_l2_mcast_addr_withindex_getnext(yt_unit_t unit, uint16_t index, uint16_t *pNext_index, l2_mcastMacAddr_info_t *pMcastMac)
{
    cmm_err_t ret = CMM_ERR_OK;
