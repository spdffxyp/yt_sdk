...
yt_phy_drv_func_t int_yt861x_drv_func =
{
    ...
    .phy_green_enable_set = int_yt861x_green_enable_set,
    .phy_green_enable_get = int_yt861x_green_enable_get,
...
yt_ret_t int_yt861x_init(yt_unit_t unit)
{
    ...
    for(phy_addr = 0; phy_addr < int_yt861x_drv_func.phy_num; phy_addr++)
    {
        ...
        {
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x50, &regData), ret);
            regData &= 0xF3FF;
            CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x50, regData), ret);
        }

        /*CSD ca*/
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x408, &regData), ret);
        regData &= 0x0FFFE;
        CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x408, regData), ret);

        /*UTP init*/
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x29, &regData), ret);
        regData &= (~(0x3F));
        regData |= 0x8;
        CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x29, regData), ret);

        /*Green Init(CLD)*/
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x3a9, &regData), ret);
        regData &= (~(0x3F));
        regData |= 0x17;
        CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x3a9, regData), ret);
        CMM_ERR_CHK(int_yt861x_green_enable_set(unit, phy_addr, YT_DISABLE), ret);
    }
...
yt_ret_t int_yt861x_green_enable_set(yt_unit_t unit, uint8_t phy_addr, yt_enable_t enable)
{
    yt_ret_t ret;
    uint16_t regData1 = 0;
    uint16_t regData2 = 0;

    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x3a0, &regData1), ret);
    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x3cc, &regData2), ret);
    if (enable == YT_ENABLE)
    {
        regData1 |= 0xC000;
        regData2 |= 0x7000;
    ...
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x3a0, regData1), ret);
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x3cc, regData2), ret);
    return CMM_ERR_OK;
}

yt_ret_t int_yt861x_green_enable_get(yt_unit_t unit, uint8_t phy_addr, yt_enable_t *pEnable)
{
    yt_ret_t ret;
    uint16_t regData1 = 0;
    uint16_t regData2 = 0;
    ...
    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x3a0, &regData1), ret);
    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x3cc, &regData2), ret);
    if (((regData1 & 0xC000) == 0xC000) && ((regData2 & 0x7000) == 0x7000))
    {
        *pEnable = YT_ENABLE;
    }
    else
...
yt_ret_t int_yt861x_parallel_detection_set(yt_unit_t unit, uint8_t phy_addr, yt_enable_t enable)
{
    CMM_UNUSED_PARAM(unit);
    CMM_UNUSED_PARAM(phy_addr);
...
yt_ret_t int_yt861x_parallel_detection_get(yt_unit_t unit, uint8_t phy_addr, yt_enable_t *pEnable)
{
    CMM_UNUSED_PARAM(unit);
    CMM_UNUSED_PARAM(phy_addr);
...
yt_ret_t int_yt861x_utp_snr_get(yt_unit_t unit, uint8_t phy_addr, yt_port_speed_t speed, yt_utp_snr_t *pSnrVal)
{
    yt_ret_t ret;
    uint16_t divisor = 0;
    uint16_t regData = 0;
    ...
    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x59, &regData1), ret);
    regData = regData1 | (1<<15);
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x59, regData), ret);
    CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0xa080, &regData2), ret);
    regData = regData2 & (~(1<<10));
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0xa080, regData), ret);
    while(loopTime < 100)
    {
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x5A, &regData), ret);
        mse0 += (regData & 0x7FFF);
        if (speed == PORT_SPEED_1000M)
        {
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x5B, &regData), ret);
            mse1 += (regData & 0x7FFF);
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x5C, &regData), ret);
            mse2 += (regData & 0x7FFF);
            CMM_ERR_CHK(int_phy_ext_reg_read(unit, phy_addr, 0x5D, &regData), ret);
            mse3 += (regData & 0x7FFF);
        }
        loopTime++;
    }
    ...
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0x59, regData1), ret);
    CMM_ERR_CHK(int_phy_ext_reg_write(unit, phy_addr, 0xa080, regData2), ret);
    return CMM_ERR_OK;
}
