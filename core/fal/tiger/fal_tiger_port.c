...
static yt_ret_t fal_rgmii_vbias_set(yt_unit_t unit, yt_dvddio_power_pad_t powerPad, yt_dvddio_power_level_t powerLevel)
{
    ...
    if ((CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218 && POWER_PAD_RGMII1 == powerPad) ||
           CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9215)
    {
        index = 1;
    }
    else if (CAL_SWCHIP_MODEL(unit) == YT_SW_MODEL_9218 && POWER_PAD_RGMII2 == powerPad)
    {
        index = 3;
    }
    else
...
static yt_ret_t fal_port_serdes_init(yt_unit_t unit)
{
    ...
    for(phyAddr = 8; phyAddr <= 9; phyAddr++)
    {
        ...
        /*vco band init*/
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phyAddr, 0x2e, &phyData), ret);
        SET_BIT(phyData, 9);
        CMM_ERR_CHK(int_phy_ext_reg_write(unit, phyAddr, 0x2e, phyData), ret);

        /*serdes as mode */
        CMM_ERR_CHK(int_phy_ext_reg_read(unit, phyAddr, 0x1d0, &phyData), ret);
        phyData &= ~(0xff << 0);
        phyData |= (0x80<<0);
        CMM_ERR_CHK(int_phy_ext_reg_write(unit, phyAddr, 0x1d0, phyData), ret);
    }
...
yt_ret_t fal_tiger_port_phyGreen_enable_set(yt_unit_t unit, yt_port_t port, yt_enable_t enable)
{
    yt_port_t phy_addr;
    yt_macid_t mac_id;
    ...
    if(phy_addr != INVALID_ID)
    {
        if(HALPHYDRV(unit, mac_id) != NULL)
        {
            HALPHYDRV_FUNC(unit, mac_id)->phy_green_enable_set(unit, phy_addr, enable);
            return CMM_ERR_OK;
        }
    }
...
yt_ret_t fal_tiger_port_phyGreen_enable_get(yt_unit_t unit, yt_port_t port, yt_enable_t *pEnable)
{
    yt_port_t phy_addr;
    yt_macid_t mac_id;
    ...
    if(phy_addr != INVALID_ID)
    {
        if(HALPHYDRV(unit, mac_id) != NULL)
        {
            HALPHYDRV_FUNC(unit, mac_id)->phy_green_enable_get(unit, phy_addr, pEnable);
            return CMM_ERR_OK;
        }
    }
 ...
 * @retval        CMM_ERR_FAIL        -on fail
 */
yt_ret_t fal_tiger_port_parallel_detection_set(yt_unit_t unit, yt_port_t port,yt_enable_t state)
{
    yt_port_t phyAddr;
    yt_macid_t mac_id;
    cmm_err_t ret = CMM_ERR_OK;
    ...
    if(phyAddr != INVALID_ID)
    {
        if (CAL_IS_SERDES(unit, port))
        {
            if(HALPHYDRV(unit, mac_id) != NULL)
            {
                HALPHYDRV_FUNC(unit, mac_id)->phy_parallel_detection_set(unit, phyAddr, state);
                return ret;
            }
        }
    }
 ...
 * @retval        CMM_ERR_FAIL        -on fail
 */
yt_ret_t fal_tiger_port_parallel_detection_get(yt_unit_t unit, yt_port_t port,yt_enable_t *pState)
{
    yt_port_t phyAddr;
    yt_macid_t mac_id;
    cmm_err_t ret = CMM_ERR_OK;
    ...
    if(phyAddr != INVALID_ID)
    {
        if (CAL_IS_SERDES(unit, port))
        {
            if(HALPHYDRV(unit, mac_id) != NULL)
            {
                HALPHYDRV_FUNC(unit, mac_id)->phy_parallel_detection_get(unit, phyAddr, pState);
                return ret;
            }
        }
    }
