...
uint32_t acl_id[YT_UNIT_NUM][YT_ACL_KEY_BIT_SIZE(ACL_IDX_MAX)];
...
yt_ret_t fal_tiger_acl_rule_create(yt_unit_t unit, uint16_t rulePri, yt_bool_t ruleReverse, uint32_t *pId)
{
    ...
    osal_memset(&vlan_data, 0, sizeof(yt_acl_key_vid_t));
    ...
    {
        fal_tiger_acl_rule_key_add(unit, YT_IGRACL_TEMPLATE_CVID, (yt_comm_key_t *)&vlan_data);
    }
    ACL_LOCK();
...
yt_ret_t fal_tiger_acl_rule_del(yt_unit_t unit, uint32_t id)
{
    ...
    {
        for (count = 0; count < YT_ACL_KEY_BIT_SIZE(ACL_IDX_MAX); count++)
        {
