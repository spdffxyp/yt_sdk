...
{                         \
        ...
        "Too less parameters",  /*CMM_ERR_TOO_LESS_INFO*/ \
        "Forbidden setting",  /*CMM_ERR_FORBIDDEN*/ \
        "Unknown error"               /* CMM_ERR_MAX */ \
}

typedef enum yt_debug_level_e
{
    YT_DEBUG_NONE = 0,
    YT_DEBUG_WARN,
    YT_DEBUG_ERROR,
...
do {\
    osal_printf("%s%s\r\n", \
              ...
              _yt_errmsg[(CMM_ERR_MAX > err_code) ? err_code : CMM_ERR_MAX]); \
} while (0)
...
#define YT_PRINT_OP(trace_level, err_code, op)\
do {\
    osal_printf("%s%s of \"%s\" in %s\r\n", \
              _yt_prompt_msg[(0 <= trace_level && YT_DEBUG_MAX > trace_level) ? trace_level : YT_DEBUG_MAX], \
              _yt_errmsg[(CMM_ERR_MAX > err_code) ? err_code : CMM_ERR_MAX], \
              #op, __FUNCTION__); \
} while (0)

#define YT_DEBUG_OP(trace_level, err_code, op)\
do {\
    if (trace_level <= yt_debug_level) {\
        \
        YT_PRINT_OP(trace_level, err_code, op); \
    }\
} while (0)
...
do {\
    if ((ret = (op)) != CMM_ERR_OK) {\
        YT_DEBUG_OP(YT_DEBUG_ERROR, ret, op); \
        return ret;\
    }\
} while(0)

#define YT_DEBUG_LEVEL(_level) \
do {\
    yt_debug_level = _level;  \
} while (0)
